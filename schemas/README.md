...
| --- | --- | --- |
| `common.schema.json` | shared defs: uuid, languageCode, manuscriptLanguage (`en`), spellingLocale, text, localizedText, lengthModel, lengthTarget, storyClock, realityFrame, evidenceRef (code points), knowerRef, stance, dialogueRegister, registerShiftReason, qualityDimension, materiality, versionRef | glossary |
...
| `chapter-contract.schema.json` | chapter acceptance unit (length in words; active constraints ref) | ADR-0013, ADR-0033 |
...
| `issue.schema.json` | evaluation finding with dimension and evidence | 05-generation/02 §1 |
| `scorecard.schema.json` | merged evaluation with separate prose / structure / genre / voice sections | 05-generation/02 |
...
| `canon-delta.schema.json` | extracted/reconciled/verified changes (incl. proposition_truth items) | FR-7.2–7.3 |
...
5. Offsets are Unicode code points into NFC text (ADR-0030).
