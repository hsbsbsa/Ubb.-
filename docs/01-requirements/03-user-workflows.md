...
1. `New Project` → enter title (working), premise (free text; Korean or English), pick genre + subgenres from
   the catalog (`docs/02-korean-style/03-genre-catalog.md`), target chapters, target chars/chapter (default
   5,500), audience/rating, operating mode (default Assisted), quality tier (default Standard).
2. Optional "Advanced" accordion: main character, supporting cast, world concept, tropes (multi-select +
   ...
   preference, mandatory scenes, content restrictions.
...
4. **Assumption Review** screen: list of assumptions grouped (genre conventions, cast, world, progression,
   romance, ending, style). Each: Confirm / Edit / Reject / Decide later. Bulk "Confirm all safe defaults".
...
1. `Build bible` → `StoryBibleWorkflow` (characters → speech profiles → world → power system → factions →
   locations → glossary → style profile binding → consistency check).
2. **Bible** screens: Characters (with speech profile editor: default speech levels toward counterparts,
   address terms, tics), World, Power System (ranks table, costs, limits, cadence), Factions, Locations,
   Glossary (fixed spellings), Style Profile (genre overlays + overrides, preview of compiled style block).
...
3. Gate (Assisted): **Chapter Review** screen — manuscript (mobile preview), scorecard, issues with evidence,
   candidate comparison (if N>1), extraction preview (proposed canon delta). Actions: Approve / Request
   changes (free text → patch tasks) / Reject (with reason) / Regenerate.
...
1. `Directions` panel: enter text; system classifies (hard/soft), scope (from chapter k / arc / character /
   series), and shows how it affects plans (re-plan preview: which contracts become stale).
...
1. **Canon Inspector** → any fact/event/knowledge/relationship item → `Correct`. Edit value/validity/
   stance; give reason.
2. System shows impact report: dependent chapters/plans (via dependency edges), affected promises,
   summaries to regenerate.
...
1. From chapter → `Regenerate` with optional directions. Dependency report shown: later accepted chapters
   that consumed canon from this chapter; author chooses: regenerate only (later chapters stale, review
   later) or cascade (queue later chapters for re-validation).
...
kind (knowledge leak, timeline, injury, inventory, rank, style drift, voice drift, repetition…).
...
- **Character**: current state (location, injuries, inventory, rank, resources), state history, arc
  progress, speech profile, voice exemplars.
- **Knowledge matrix**: propositions × knowers with stance icons; "as of chapter k" slider.
- **Relationships**: graph + directed pair detail (type, trust, address terms, speech level, history).
- **Promises**: open/overdue/paid; due windows; linked setups/payoffs.

## UW-14 Review costs

Project cost page: spend by role/model/chapter, cost per accepted chapter, per 1,000 accepted characters,
...
(chapter header format, glossary appendix, AI disclosure text, rights statement). Export job produces a
file in object storage with a signed URL; export recorded in audit log.
