...
| FR-4.5 | Candidate generation (N≥2) mechanism ships in MVP and is configurable per project: concepts (always N≥2), arc plans (Standard+), scenes/chapters (Premium tier or on user request in MVP; default-on for Standard in Beta). Candidates are judged pairwise with position swapping; generation stops early when a candidate exceeds the early-stop threshold or budget is reached. | M/P0 (mechanism), B/P1 (Standard default-on) |
