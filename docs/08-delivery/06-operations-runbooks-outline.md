...
| **Canon repair** | disagreement spike / user report | reading a chapter's trace, rolling back the latest commit, running correction workflow, regenerating summaries and embeddings for a range |
| **Prompt/model change** | new version | regression suite run, calibration check on contrast pairs, staged promotion, monitoring quality metrics for 24h |
